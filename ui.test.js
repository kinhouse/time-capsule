...
describe('geolocation auto-fill on page load', () => {
  function buildLocationDOM() {
    document.body.innerHTML = `<input type="text" id="location" />`
  }

  function initGeo(geolocation) {
    const locationInput = document.getElementById('location')
  ...
  it('does nothing when geolocation API is unavailable', () => {
    buildLocationDOM()
    initGeo(null)
    expect(document.getElementById('location').value).toBe('')
  })
